...
def get_selected_curtain_wall():
    ...
    try:
        ...
        if not selection:
            forms.alert('Please select a curtain wall.', exitscript=True)
            return None
...
def get_grid_spacing(wall):
    ...
    try:
        ...
        # Convert to feet and validate
        spacing = grid_param.AsDouble()
        if spacing <= 0:
            forms.alert('Vertical grid spacing must be greater than zero.', exitscript=True)
            return None
            
        return spacing
        
    except Exception as e:
        forms.alert('Error getting grid spacing: {}\nPlease check wall type parameters.'.format(str(e)), exitscript=True)
        return None
...
def resize_curtain_wall(wall, door_count, maintain_center):
    ...
    try:
        ...
        with Transaction(doc, 'Resize Curtain Wall') as t:
            ...
            if maintain_center and current_length > 0:  # Add check for division by zero
                # Find middle point
        ...
        forms.alert('Error resizing curtain wall: {}\nPlease check wall geometry.'.format(str(e)), exitscript=True)
